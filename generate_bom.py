...
def get_github_repo_info():
    ...
    try:
        # Tente de récupérer l'URL du dépôt distant via git
        remote_url = subprocess.check_output(["git", "config", "--get", "remote.origin.url"]).decode().strip()
        # Transforme https://github.com en USER/REPO
        ...
        # Valeur par défaut si git n'est pas disponible (à personnaliser si besoin)
        return "."
...
def generate_bom():
    ...
    # Récupération de l'URL pour le téléchargement direct
                ...
                for item in sorted(list(module.rglob("*"))):
                    if item.is_dir() or item.suffix.lower() == ".stl":
                        ...
                        if item.suffix.lower() == ".stl":
                            ...
                            view = f"[👁️]({url_path})"
                            # LIEN DE TÉLÉCHARGEMENT CORRIGÉ (RAW URL)
