...
def generate_bom():
    ...
    md = ["# 📋 Nomenclature (BOM)\n", "## 📌 Sommaire"]
    for m, _ in sections:
        # Ancre GitHub : minuscule, espaces -> tirets, pas d'émojis
    ...
    for mod, parent in sections:
        ...
        for item in sorted(mod.rglob("*")):
            ...
            # CRUCIAL : safe='/' pour garder les dossiers dans l'URL
            ...
            if item.is_file():
                ...
                md.append(f"| [🔍 Voir]({blob_url}/{u_path}) | {indent}📄 {item.name} | {'🟢' if old_val else '🔴'} | {old_val or '---'} | [💾]({raw_url}/{u_path}) |")
            else:
    ...
    Path(SETTINGS_FILE).write_text(json.dumps(new_data, indent=4), encoding="utf-8")
    print("✅ Terminé.")
